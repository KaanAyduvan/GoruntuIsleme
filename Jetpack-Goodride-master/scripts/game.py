...
class Game:
    ...
    def update_game(self, main):
        ...
        # 🔁 Jetpack Gesture komutları kontrolü (debounce ile)
        gesture_cmd = self.main.gesture.pop()

        # ⏳ Timeout gesture bekleme süresi
        if hasattr(self, "pause_gesture_cooldown"):
            if self.pause_gesture_cooldown > 0:
                self.pause_gesture_cooldown -= self.main.dt
        else:
            self.pause_gesture_cooldown = 0  # ilk kullanımda tanımla

        if gesture_cmd == "timeout" and self.pause_gesture_cooldown <= 0:
            self.paused = not self.paused
            self.pause_gesture_cooldown = 1.2  # tekrar timeout kabulü için 1.2s bekle
            if self.paused:
                pygame.mixer.music.pause()
            else:
                pygame.mixer.music.unpause()

        elif gesture_cmd == "open" and not self.paused:
            self.is_moving_up = True
        elif gesture_cmd == "fist" and not self.paused:
            self.is_moving_up = False
